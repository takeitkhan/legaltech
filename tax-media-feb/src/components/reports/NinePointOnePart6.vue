<template>
  <div class="taxpayer-information">
    ...
    <div class="taxpayer-information-table">
      ...
      >
        ...
        <tr>
          ...
          <th style="text-align: center">
            <a
              @click.prevent="GoodsDetails('SubFormForTwentyNine', {note: 10})"
            >Sub-form</a>
          </th>
        </tr>
        ...
        <tr>
          ...
          <th style="text-align: center">
            {{ TOTAL_AT_VALUE_GETTERS }}
          </th>
          <th style="text-align: center">
            <a
              @click.prevent="
                GoodsDetails('SubFormForNoteThirty', {note: 30})"
            >Sub-form</a>
          </th>
        </tr>
        <!-- .single nature  -->
    ...
    <VATAdjustmentModal
      ...
      title="Add Decreasing Adjustment"
      buttontext="Close"
      :form="form"
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('reportninepointone', {
      TOTAL_AT_VALUE_GETTERS: 'TOTAL_AT_VALUE_GETTERS',
    }),
    getsdRate() {
      return trans => {
  ...
  methods: {
    ...
    async getAdjustments() {
      try {
        ...
        if (this.reportDate) {
          const { data: { adjustments } } = await EntityApi.getAdjustmentsInDate(this.currentEntity.id, this.reportDate, 'decreasing')
          this.adjustments = adjustments
        } else if (Array.isArray(this.reportDateRange)) {
          /// reportDate range has date
          const { data: { adjustments } } = await EntityApi.getAdjustmentsViaDateRange(this.currentEntity.id, this.reportDateRange[0], this.reportDateRange[1], 'decreasing')
          this.adjustments = adjustments
        }
      } catch (error) {
        console.log(error)
